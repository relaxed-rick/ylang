...
- Builds as a Chrome/Chromium MV3 extension and an experimental Firefox MV3 package.
- Includes NRK support, experimental Netflix page-exposed timed subtitle loading, and rendered-subtitle fallback adapters for Netflix, YouTube, and ORF ON.
...
- Site support is partly adapter-specific. NRK is the most complete adapter; Netflix can try subtitles exposed to the active Netflix page and falls back to rendered subtitle observation; YouTube and ORF ON currently rely on rendered subtitle observation; generic native HTML5 track probing exists in code but is not broadly injected on every website yet.
...
npm run release:check:firefox
npm run public:check
```

For release-readiness checks, run typecheck, build, release checks, and public boundary check as separate commands. `npm run build` creates a root Chromium-compatible `dist` package plus browser-specific packages at `dist/chromium` and `dist/firefox`.
...
4. Select the `dist` folder, or `dist/chromium` if you want the explicit Chromium package.
...
Firefox packaging is experimental.

1. Run `npm run build`.
2. Open `about:debugging#/runtime/this-firefox`.
3. Choose **Load Temporary Add-on**.
4. Select `dist/firefox/manifest.json`.
5. Run the manual smoke tests from `docs/smoke-test-checklist.md`.

## Notes

- The tool does not ship, host, or distribute subtitles.
- Netflix support does not bypass DRM, authentication, paywalls, or access controls. It only uses subtitle metadata/text that the active logged-in page exposes while the user is watching, then falls back to visible rendered subtitles if timed text is unavailable.
...
- `dist`, `dist/chromium`, and `dist/firefox` are generated by `npm run build`. They are unpacked extension outputs, not the source of truth, and should normally be rebuilt rather than edited.
...
- License, setup, release planning, changelog, change intake, contribution notes, private-alpha stabilization, public-repo boundaries, browser compatibility, privacy boundaries, permissions, third-party notices, player support, troubleshooting, local LLM setup, Anki setup, data storage/backup, and smoke testing live in `LICENSE.md`, `CHANGELOG.md`, `docs/change-intake.md`, `CONTRIBUTING.md`, `docs/license-notes.md`, `docs/release-plan.md`, `docs/private-alpha-stabilization.md`, `docs/public-repo-boundary.md`, `docs/browser-compatibility.md`, `docs/privacy.md`, `docs/permissions.md`, `docs/third-party-notices.md`, `docs/player-support.md`, `docs/troubleshooting.md`, `docs/llm-setup.md`, `docs/anki.md`, `docs/data-storage-and-backup.md`, `docs/smoke-test-checklist.md`, and `docs/smoke-test-results-template.md`.
