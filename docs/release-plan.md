...
This is the working checklist for publishing ylang as an open-source browser extension on GitHub. Browser-store publishing is a later, stricter track.
...
- Run `npm run typecheck`, `npm run build`, `npm run release:check`, and `npm run public:check` as separate commands.
...
- Verify `dist/manifest.json` contains no unexpected permissions or generated source-only paths.
...
- Store release packaging: zip only the built extension output, not source, node_modules, or local artifacts.
